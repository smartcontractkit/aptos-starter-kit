...
export async function fetchEventsByTxHash(txHash: string, aptos: Aptos) {
  ...
  try {
    ...
    if (transaction.type === 'user_transaction') {
      const ccipSendEvent = transaction.events.filter((event) =>
...
export function encodeGenericExtraArgsV2(
  gasLimit: bigint,
  allowOutOfOrderExecution: boolean
): Uint8Array {
  // Initialize an empty array to store the encoded bytes
  const extraArgs: number[] = [];
...
export function parseAmountToU64Decimals(
